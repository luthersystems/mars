...
class Terraform(object):
    ...
    def main(self):
        ...
        argparser.add_argument(
            "--skip-prompt", action="store_true", help="Skip workspace switch prompt"
        )
        ...
        init_parser.add_argument(
            "--reconfigure", action="store_true", help="Reconfigure the backend on init"
        )
        ...
        taint_parser.add_argument(
            "--module", help="Module containing the resource to taint"
        )
        ...
        untaint_parser.add_argument(
            "--module", help="Module containing the resource to untaint"
        )
        ...
        migration_fromplan_parser.add_argument(
            "tfplan_file", help="input from terraform plan -out"
        )
        migration_fromplan_parser.add_argument(
            "migration_file", help="output HCL migration file"
        )
    ...
    def init(self, backend_config=None, upgrade=None, reconfigure=None):
        self._tfenv_init()
        args = []
    ...
    def terraform(self, args):
        ...
        cmd = ["terraform"]
        cmd.extend(args)
        rc = self._script(self._tf_workspace_select(), cmd)
        if rc != 0:
            exit(rc)
    ...
    def plan(
        ...
        if apply_plan and plan_path is None:
            ...
            plan_file, plan_path = tempfile.mkstemp(
                prefix=plan_name_prefix, suffix=".out", dir=plan_dir
            )
            os.close(plan_file)
        ...
        rc = self._script(self._tf_workspace_select(), args)
        if apply_plan:
            if rc != 0:
    ...
    def apply(self, plan=None, target=None, refresh_only=None, approve=None):
        ...
        rc = self._script(
            self._tf_workspace_select(), ["terraform", "apply"] + list(args)
        )
        exit(rc)
    ...
    def destroy(self, approve=None):
        ...
        if approve:
            args.append("-auto-approve")
        rc = self._script(
            self._tf_workspace_select(), ["terraform", "destroy"] + list(args)
        )
        exit(rc)
    ...
    def show(self, plan=None):
        ...
        if plan:
            args = [plan]
        rc = self._script(
            self._tf_workspace_select(), ["terraform", "show"] + list(args)
        )
        exit(rc)
    ...
    def graph(self, draw_cycles=None):
        ...
        if draw_cycles:
            args = ["-draw-cycles"]
        rc = self._script(
            self._tf_workspace_select(), ["terraform", "graph"] + list(args)
        )
        exit(rc)
    ...
    def import_action(self, allow_missing_config=None, addr=None, resource_id=None):
        ...
        args = itertools.chain(
            base_args, var_file_args, extra_args, [addr, resource_id]
        )
        rc = self._script(self._tf_workspace_select(), args)
        exit(rc)
    ...
    def _locate_var_files(self):
        if self._var_files is None:
            common_files = self._all_var_files(
                os.path.join(self.var_dir, self.shared_env)
            )
            common_files = [
                os.path.join(self.shared_env, path) for path in common_files
            ]
    ...
    def _sh(self, cmd):
        process = subprocess.Popen(
            ["/bin/bash", "-c", cmd], stdin=sys.stdin, stderr=sys.stderr
        )
        process.communicate()
        rc = process.wait()
        return rc
