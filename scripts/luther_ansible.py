...
class Ansible(object):
    ...
    def main(self):
        ...
        vault_parser.add_argument("--aws-sm-secret-id", default="")
        vault_parser.add_argument("--aws-region", default="")
        vault_parser.add_argument("--aws-role-arn", default="")

        playbook_parser = subparsers.add_parser(
            "ansible-playbook", parents=[vault_parser]
        )
        ...
        execute_parser = subparsers.add_parser(
            "ansible-execute", parents=[vault_parser]
        )
        ...
        vault_encrypt_parser = subparsers.add_parser(
            "ansible-vault-encrypt", parents=[vault_parser]
        )
        vault_encrypt_parser.add_argument("--path", default="")
        vault_encrypt_parser.set_defaults(func=self.vault_encrypt)

        vault_decrypt_parser = subparsers.add_parser(
            "ansible-vault-decrypt", parents=[vault_parser]
        )
        vault_decrypt_parser.add_argument("--path", default="")
        vault_decrypt_parser.set_defaults(func=self.vault_decrypt)

        vault_decrypt_key_parser = subparsers.add_parser(
            "ansible-vault-decrypt-key", parents=[vault_parser]
        )
        ...
        if any(aws_sm_args) and not all(aws_sm_args):
            raise Exception(
                "--aws-sm-secret-id and --aws-region must be supplied together"
            )
    ...
    def playbook(self, args):
        ...
        cmd = itertools.chain(
            base_cmd, vault_args, inv_args, user_args, ssh_common_args, extra_args
        )
    ...
    def _get_encryption_key(self, args):
        if args.az_vault:
            return keyvault.get_azure_secret(args.az_vault, args.az_vault_key)
        if args.aws_sm_secret_id:
            return keyvault.get_aws_secret(
                args.aws_sm_secret_id, args.aws_region, role_arn=args.aws_role_arn
            )
    ...
    def _ansible_vault_args(self, args):
        ...
        if args.aws_sm_secret_id:
            return ["--vault-id", "/opt/mars/vault-aws-secretsmanager.py"]
        if self.vault_password_file is None:
            self.vault_password_file = self._find_ansible_vault_password_file()
    ...
    def _find_ansible_vault_password_file(self):
        ...
        if len(files) == 0:
            raise Exception(
                "no remote vault key supplied and vault password file could not be located"
            )
        if len(files) > 1:
            raise Exception(
                "unable to determine vault password file from ambiguous entries {}".format(
                    files
                )
            )
        return files[0]
...
def vault_client(encryption_key):
    return VaultLib(
        [(DEFAULT_VAULT_ID_MATCH, VaultSecret(bytes(encryption_key, "utf-8")))]
    )
...
def _add_env_vars(args):
    ...
    if args.aws_sm_secret_id:
        return {
            "AWS_SM_SECRET_ID": args.aws_sm_secret_id,
